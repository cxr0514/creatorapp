      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-gray-900">{mockClip.title}</h3>
            <p className="text-gray-600 mt-2">{mockClip.description}</p>
            <div className="flex flex-wrap gap-2 mt-3">
              {mockClip.hashtags.map(tag => (
            ...
            <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
              <span>Duration: {Math.floor(mockClip.duration / 60)}:{(mockClip.duration % 60).toString().padStart(2, '0')}</span>
              <span>Aspect Ratio: {mockClip.aspectRatio}</span>
            </div>
          </div>
